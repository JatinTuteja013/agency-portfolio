...
export default function SiteNav({ light = false, iconDark = false }: SiteNavProps) {
  ...
  const [mobileOpen, setMobileOpen] = useState(false);
  const [brandsOpen, setBrandsOpen] = useState(false);
  // Ref now wraps the ENTIRE nav (desktop + mobile), not just the desktop <li>.
  // Previously this was scoped to the desktop dropdown only, so any click on
  // the mobile "Brands" button/links was treated as an "outside" click and
  // closed the dropdown on mousedown, before the click/navigation could fire.
